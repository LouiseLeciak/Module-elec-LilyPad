...
void draw_char_small(uint16_t x, uint16_t y, uint8_t c, uint16_t fg,
                     uint16_t bg, uint8_t ScaleFactor)
{
  if (c < 32 || c > 126)
    c = '?';

  const uint8_t* char_in_array =
      font5x7[c - 32];  // pointer to the 5-byte char_in_array

  for (int8_t col = 0; col < 5; col++)
  {
    uint8_t columnsBits =
        pgm_read_byte(&char_in_array[col]);  // get char array colums bits
    ...
    for (int8_t row = 0; row < 7; row++)
    {
      ...
      for (int8_t i = 0; i < ScaleFactor; i++)
      {
        ...
        {
          // if (!(columnsBits & (1 << row))) continue;   //OPTION add if you
          // want to have transparentbackground skip pixels

          draw_pixel((x + (col * ScaleFactor) + z),
                     (y + (row * ScaleFactor) + i), color);
        }
      }
    }
  }
...
void draw_string(uint16_t x, uint16_t y, const char* str, uint16_t fg,
                 uint16_t bg, uint8_t ScaleFactor, uint8_t HGap)
{
