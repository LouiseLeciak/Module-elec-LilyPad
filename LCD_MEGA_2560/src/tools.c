...
extern volatile char
    nbr_in_a_string[33]; // global variable for function nbr_to_str()

bool
is_upper (unsigned char c)
{
...
bool
ft_strcmp (unsigned char *str, unsigned char *name)
{
...
bool
is_len_size (unsigned char *str, int size)
{
...
bool
is_first_char (unsigned char *str, unsigned char c)
{
...
bool
is_valid_hex_str (unsigned char *str)
{
...
void
split_hex (unsigned char *str, unsigned char *hex, int start_pos)
{
...
/// REF: - str not protected in size. undefined result if under or over size
/// string

uint8_t
convert_from_hex (unsigned char *str)
{
...
void
reverseNumString (volatile char *str)
{
...
void
ft_itoa (uint32_t nbr, int index)
{
...
void
loop_for_one_millisecond (volatile uint16_t loop_Max)
{
...
void
pause_in_milliseconds (uint16_t time_in_ms)
{
...
uint16_t
nbrStr_to_dec (const char *nbrString)
{
...
bool
is_digit_str (volatile char *str)
{
...
uint32_t
nbrStr_to_dec_32t (const char *nbrString)
{
...
int16_t
nbrStr_to_dec_signed (const char *nbrString)
{
  int32_t result = 0;
  uint8_t index = 0;
  int8_t neg = 1;
  ...
  while (nbrString[index])
    {
      ...
      if (neg == 1 && result > 32767)
        {
          return 32767;
        }
      if (neg == -1 && result > 32768)
        {
          return -32768;
        }
